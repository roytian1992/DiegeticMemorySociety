...
from dms.parsing import extract_json_value, validate_scene_event_candidates
...
class SceneEventRunConfig:
    script_path: Path
    output_dir: Path
    prompt_dir: Path = Path("task_specs/prompts")
    task_settings_path: Path = Path("task_specs/task_settings/scene_event_candidates_task.json")
...
def run_scene_events(config: SceneEventRunConfig, *, llm_client: LLMClient | None = None) -> dict[str, Any]:
    """Render and optionally execute scene-event candidate extraction prompts."""
    ...
    prior_context = _read_prior_context(config.prior_context_path)
    ...
    for ordinal, scene in enumerate(selected, start=1):
        ...
        prompt_text = loader.render(
            prompt_spec,
            task_values={"unit_json": unit_json, "prior_context": prior_context},
        ...
        if config.dry_run:
            ...
            try:
                ...
                validation_errors = (
                    validate_scene_event_candidates(parse_result.data, expected_scene_id=scene.scene_id)
                    if parse_result.ok
                    else []
                )
    ...
    manifest = {
        "run_type": "scene_event_candidates",
        ...
        "prior_context_path": str(config.prior_context_path.resolve()) if config.prior_context_path else None,
        ...
        "config": {
            **asdict(config),
            ...
            "prior_context_path": str(config.prior_context_path) if config.prior_context_path else None,
        },
    }
    summary = {
        "run_type": "scene_event_candidates",
...
def _read_prior_context(path: Path | None) -> str:
    if path is None:
        return ""
    if not path.exists():
        raise FileNotFoundError(f"Prior context path not found: {path}")
    if path.is_dir():
        summary_path = path / "summary.json"
        if summary_path.is_file():
            return summary_path.read_text(encoding="utf-8")
        memory_path = path / "canonical_memory.json"
