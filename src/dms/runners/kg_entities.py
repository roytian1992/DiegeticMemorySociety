...
from dms.parsing import extract_json_value, validate_kg_entity_mentions
...
class KGEntityRunConfig:
    script_path: Path
    output_dir: Path
    prompt_dir: Path = Path("task_specs/prompts")
    task_settings_path: Path = Path("task_specs/task_settings/kg_entity_mentions_task.json")
...
def run_kg_entity_mentions(config: KGEntityRunConfig, *, llm_client: LLMClient | None = None) -> dict[str, Any]:
    """Render and optionally execute KG entity mention extraction prompts."""
    ...
    entity_type_policy = _format_policy(task_settings.get("entity_type_policy", []))
    prior_entity_context = format_author_entity_context_for_prompt(load_author_entity_context(config.prior_entity_context_path))
    ...
    for ordinal, scene in enumerate(selected, start=1):
        ...
        prompt_text = loader.render(
            prompt_spec,
            task_values={"unit_json": unit_json, "prior_entity_context": prior_entity_context},
            static_values={
        ...
        if config.dry_run:
            ...
            try:
                ...
                validation_errors = (
                    validate_kg_entity_mentions(parse_result.data, expected_scene_id=scene.scene_id)
                    if parse_result.ok
                    else []
                )
    ...
    manifest = {
        ...
        "prior_entity_context_path": str(config.prior_entity_context_path.resolve())
        if config.prior_entity_context_path
        else None,
        ...
        "config": {
            **asdict(config),
            ...
            "prior_entity_context_path": str(config.prior_entity_context_path) if config.prior_entity_context_path else None,
        },
    }
    summary = {
        "run_type": "kg_entity_mentions",
