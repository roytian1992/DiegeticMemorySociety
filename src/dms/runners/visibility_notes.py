...
from dms.parsing import extract_json_value, validate_visibility_notes
...
class VisibilityNotesRunConfig:
    script_path: Path
    output_dir: Path
    extracted_candidates_dir: Path
    prompt_dir: Path = Path("task_specs/prompts")
    task_settings_path: Path = Path("task_specs/task_settings/visibility_notes_task.json")
    start: int = 1
    limit: int = 5
    dry_run: bool = True
...
def run_visibility_notes(config: VisibilityNotesRunConfig, *, llm_client: LLMClient | None = None) -> dict[str, Any]:
    """Render and optionally execute scene-level visibility note prompts."""
    ...
    if not config.extracted_candidates_dir.is_dir():
        raise FileNotFoundError(f"Extracted candidates dir not found: {config.extracted_candidates_dir}")
    ...
    for ordinal, scene in enumerate(selected, start=1):
        ...
        candidates = _read_candidates(config.extracted_candidates_dir, scene.scene_id)
        candidates_json = json.dumps(candidates, ensure_ascii=False, indent=2)
        prompt_text = loader.render(
            prompt_spec,
            task_values={"unit_json": unit_json, "extracted_candidates": candidates_json},
        ...
        if config.dry_run:
            ...
            try:
                ...
                validation_errors = (
                    validate_visibility_notes(parse_result.data, expected_scene_id=scene.scene_id)
                    if parse_result.ok
                    else []
                )
    ...
    manifest = {
        ...
        "config": {
            **asdict(config),
            ...
            "extracted_candidates_dir": str(config.extracted_candidates_dir),
        },
    }
    summary = {
        "run_type": "visibility_notes",
...
def _read_candidates(candidates_dir: Path, scene_id: str) -> dict[str, Any]:
    direct_path = candidates_dir / f"{scene_id}.json"
    parsed_path = candidates_dir / "parsed" / f"{scene_id}.json"
    if direct_path.is_file():
        payload = _read_json(direct_path)
    elif parsed_path.is_file():
        payload = _read_json(parsed_path)
    else:
        raise FileNotFoundError(f"No extracted candidates found for {scene_id} under {candidates_dir}")
