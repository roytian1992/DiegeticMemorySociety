...
def build_memory_packet(config: MemoryPacketConfig) -> dict[str, Any]:
    ...
    return {
        "retrieval_boundary": {
            ...
            "semantics": "strictly_before_scene_when_before_scene_id_is_used",
        },
...
def format_memory_packet_markdown(packet: dict[str, Any]) -> str:
    ...
    boundary = (packet.get("retrieval_boundary") or {}).get("before_scene_id")
    state_label = f"current state before {boundary}" if boundary else "current state"
...
def _entity_context(
    ...
    if not memories:
        return {
            "profile": f"{name}：{entity_type_label}；检索边界前没有关联到可用的 episodic memory。{alias_text}".strip(),
    ...
    return {
        "profile": f"{name}：{profile_text or entity_type_label}。{alias_text}".replace("。。", "。").strip(),
