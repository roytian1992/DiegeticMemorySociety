...
def test_scene_events_dry_run_writes_artifacts(tmp_path: Path) -> None:
    output_dir = tmp_path / "scene_events"
    ...
    assert "Fill the event candidate JSON" in prompt_text
    assert "scene_0001" in prompt_text


def test_scene_events_fake_llm_run_parses_outputs(tmp_path: Path) -> None:
    output_dir = tmp_path / "fake_events"
    summary = run_scene_events(
        SceneEventRunConfig(
            script_path=SCRIPT_PATH,
            output_dir=output_dir,
    ...
    assert '"FAKE_EVENT"' in parsed


def test_scene_events_requires_llm_client_for_non_dry_run(tmp_path: Path) -> None:
    try:
        run_scene_events(
            SceneEventRunConfig(
                script_path=SCRIPT_PATH,
